...
ZSH_THEME_GIT_PROMPT_AHEAD="%{$cyan_bold%}↑"
ZSH_THEME_GIT_PROMPT_BEHIND="%{$cyan_bold%}↓"
ZSH_THEME_GIT_PROMPT_DIVERGED="%{$cyan_bold%}⇕"
...
function get_git_prompt {
  if [[ -n $(git rev-parse --is-inside-work-tree 2>/dev/null) ]]; then
    ...
    local git_prompt=" %{$magenta%}^ %{$reset_color%}$(git_prompt_info)$git_status"
    echo $git_prompt
  fi
}
...
function print_prompt_head {
  local left_prompt="\
%{$blue%}# \
%{$cyan_bold%}$(get_usr_name)%{$reset_color%}\
%{$cyan%}@\
%{$cyan%}$(get_box_name): \
%{$green_bold%}$(get_current_dir)%{$reset_color%}\
$(get_git_prompt) "
